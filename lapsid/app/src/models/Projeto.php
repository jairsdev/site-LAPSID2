...
class Projeto {
    ...
    public function criar_tabela() {
        $query = "INSERT INTO projetos (titulo, conteudo, autor_id, data_criacao, data_atualizacao) VALUES (:titulo, :conteudo, :autor_id, :data_criacao, :data_atualizacao)";
    ...
    public function ler_registro() {
        $query = "SELECT * FROM projetos WHERE id = :id ORDER BY id DESC";
        $stmt = $this->conn->prepare($query);
    ...
    public function atualizar_registro() {
        $query = "UPDATE projetos SET titulo = :titulo, conteudo = :conteudo, autor_id = :autor_id, data_atualizacao = :data_atualizacao WHERE id = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindValue(":id", $this->id, PDO::PARAM_INT);
        $stmt->bindValue(":titulo", $this->titulo, PDO::PARAM_STR);
        $stmt->bindValue(":conteudo", $this->conteudo, PDO::PARAM_STR);
